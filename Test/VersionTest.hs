...
versionTests = test [ 
    ...
    "test A01"    ~: "versionToString x"                        ~: "x"                                                        ~=? ( versionToString $ Version $ VersionNumber [Nothing] ),
    "test A02"    ~: "versionToString x.x.x"                    ~: "x.x.x"                                                    ~=? ( versionToString $ Version $ VersionNumber [Nothing, Nothing, Nothing] ),
    "test A03"    ~: "versionToString Dev/x.x.x"                ~: "Dev/x.x.x"                                                ~=? ( versionToString $ MaturityVersion Dev $ VersionNumber [Nothing, Nothing, Nothing] ),
    "test A04"    ~: "versionToString Prod/1.0.3"               ~: "Prod/1.0.3"                                                ~=? ( versionToString $ MaturityVersion Prod $ VersionNumber [Just 1, Just 0, Just 3] ),
    ...
    {-"test C01"    ~: "JSON.FromJSON Test/1.0.x"                ~: MaturityVersion Test ( VersionNumber (Number 1) (VersionNumber (Number 0) (VersionCompound (NumberPlaceholder ) ) ) )    ~=? ( parseVersionFromString "{\"version\": \"Test/1.0.x\"}" ),-}
    {-"test C02"    ~: "JSON.FromJSON 2.x"                        ~: Version (VersionNumber (Number 2) (VersionCompound (NumberPlaceholder ) ) )    ~=? ( parseVersionFromString "{\"version\": \"2.x\"}" ),-}
    ...
    "test F02"    ~: "incrementDim 0 Dev/x"                     ~: "Test/x"                                                    ~=? ( versionToString (incrementDimension 0 $ stringToVersion "Dev/x" ) ),
    "test F03"    ~: "incrementDim 1 Dev/1.0"                   ~: "Dev/1.1"                                                ~=? ( versionToString (incrementDimension 1 $ stringToVersion "Dev/1.0" ) ),
    "test F04"    ~: "incrementDim 3 12.0.3"                    ~: "13.0.3"                                                    ~=? ( versionToString (incrementDimension 3 $ stringToVersion "12.0.3" ) ),
    ...
    "test H02"    ~: "decrementDim 0 Test/1.x.2"                ~: "Dev/1.x.2"                                                ~=? ( versionToString (decrementDimension 0 $ stringToVersion "Dev/1.x.2" ) ),
    "test H03"    ~: "decrementDim 2 User/1.3.2"                ~: "User/1.2.2"                                                ~=? ( versionToString (decrementDimension 2 $ stringToVersion "User/1.3.2" ) ),
